...
This creates a `.cofounder/` team skeleton, adds an internal `.cofounder/.gitignore` for run/worktree artifacts, adds `.cofounder/project.md` for shared worker project rules, adds Codex-facing project instructions when `AGENTS.md` is not already present, and can install the Codex MCP entry.
