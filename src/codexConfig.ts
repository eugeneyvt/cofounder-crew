...
function normalizeMcpOauthCredentialsStore(settings: MemberSettings): string | null {
  const value = settings.mcp?.oauth_credentials_store;
...
function buildCodexConfig(
  ...
  oauthCredentialsStore: string | null
): string {
...
function buildMcpOverrideArgs(selectedServers: Record<string, Record<string, unknown>>, oauthCredentialsStore: string | null): string[] {
  const args: string[] = [];
  if (oauthCredentialsStore) {
    args.push("-c", `mcp_oauth_credentials_store="${oauthCredentialsStore}"`);
  }
